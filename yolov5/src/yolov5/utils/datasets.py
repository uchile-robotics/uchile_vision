...
class LoadImages:  # for inference
	def __init__(self, path, img_size=640,cfg=None):
	...
	def __next__(self):
		...
		if self.cfg:
			if len(self.cfg['bbox']) != 0:
				bbox = self.cfg['bbox'][0]
				img1 = img0[bbox[1]:bbox[3],bbox[0]:bbox[2],:]
			else:
				img1=img0.copy()    
		else:
			img1=img0.copy()
		...
		return self.nick, img, img0, img1, self.cap
...
class LoadStreams:  # multiple IP or RTSP cameras
	def __init__(self, sources='streams.txt', img_size=640, cfg=None):
		self.mode = 'images'
		self.img_size = img_size
		self.cfg = cfg
		...
		self.paths = []
		for idx, dhd in enumerate(self.sources):
			self.paths.append(self.cfg[idx]['nick_cam'])
	...
	def __next__(self):
		...
		if self.cfg:
			for idx, x in enumerate(img0):
				if len(self.cfg[idx]['bbox']) != 0:
					bbox = self.cfg[idx]['bbox'][0]
					img1.append(img0[idx][bbox[1]:bbox[3],bbox[0]:bbox[2],:])  
				else:
					img1=img0.copy()    
		else:
			img1=img0.copy() 
		...
		try:
			al = self.cfg[idx]['alpha']
			bb = self.cfg[idx]['betha']
		except:
			al = None
			bb = None
		# Letterbox
		img = [letterbox(x, new_shape=self.img_size, auto=self.rect,alpha=al,betha=bb)[0] for x in img1]
		...
		return self.paths, img, img0, img1, self.cap
