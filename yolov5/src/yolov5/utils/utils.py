...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    print('Best Possible Recall (BPR) = %.4f' % bpr)
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False):
    # Returns the IoU of box1 to box2. box1 is 4, bo
    x2 is nx4
    box2 = box2.t()
...
class BCEBlurWithLogitsLoss(nn.Module):
    ...
    def __init__(self, alpha=0.05):
        ...
        sel
        f.loss_fcn = nn.BCEWithLogitsLoss(reduction='none')  # must be nn.BCEWithLogitsLoss()
        self.alpha = alpha
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [255, 255, 255], thickness=1, lineType=cv2.LINE_AA)
